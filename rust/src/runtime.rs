...
impl RuntimeWorker {
    ...
    fn synthesize(&mut self, request: &SynthesisRequest) -> Result<SynthesisResult> {
        ...
        let timings = InferenceTimings {
            ...
            alignment_build_ms: trace.timings.alignment_build_ms,
            alignment_matmul_ms: trace.timings.alignment_matmul_ms,
        ...
        for (index, chunk_phonemes) in chunks.into_iter().enumerate() {
            ...
            let diagnostics = if is_final {
                let timings = InferenceTimings {
                    ...
                    alignment_build_ms: pipeline_timings.alignment_build_ms,
                    alignment_matmul_ms: pipeline_timings.alignment_matmul_ms,
