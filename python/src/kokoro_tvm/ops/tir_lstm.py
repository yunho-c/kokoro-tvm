...
def create_tir_lstm_primfunc(
    ...
    c_new = ib.allocate(dtype, batch_size * hidden_size, "c_new", scope="global")
    h_new = ib.allocate(dtype, batch_size * hidden_size, "h_new", scope="global")
    ...
    with ib.while_loop(t[0] < seq_len):
        ...
        # PASS 1: Compute c_new and h_new into temp buffers (using OLD c values)
        with ib.for_range(0, batch_size, name="b2") as b:
            with ib.for_range(0, hidden_size, name="j2") as j:
                ...
                # Gate indices
                i_idx = b * gate_size + j
                f_idx = b * gate_size + hidden_size + j
                g_idx = b * gate_size + 2 * hidden_size + j
                o_idx = b * gate_size + 3 * hidden_size + j

                # Gate values
                i_g = tir.sigmoid(gates[i_idx])
                f_g = tir.sigmoid(gates[f_idx])
                g_g = tir.tanh(gates[g_idx])
                o_g = tir.sigmoid(gates[o_idx])

                # Compute into temp buffers (c[flat] is still the OLD value here)
                c_new[flat] = f_g * c[flat] + i_g * g_g
                h_new[flat] = o_g * tir.tanh(c_new[flat])

        # PASS 2: Copy temp values to actual buffers and output
        with ib.for_range(0, batch_size, name="b3") as b:
            with ib.for_range(0, hidden_size, name="j3") as j:
                ...
                out_ptr[out_idx] = h_new[flat]

        # Increment timestep
        t[0] = t_val + 1

    # Copy final states to output buffers
    with ib.for_range(0, batch_size, name="b4") as b:
        with ib.for_range(0, hidden_size, name="j4") as j:
            flat = b * hidden_size + j
    ...
    func = (
        ...
        .with_attr("global_symbol", "tir_lstm_forward")
